...
## Read data
```{r}
#ad <- read_h5ad(paste0("../../data/raw_data/", params$raw_data))
#dat <- ad$X %>% as.data.frame() %>% as.matrix() %>% t()

#dat%>% dim #19093 16382
#save(dat, file = str_c("../../data/derived_data/",  str_c("../../data/derived_data/",
 #                           str_replace(params$raw_data, pattern = ".h5ad", replacement = ""), "dat.RData"))
list.files(str_c("../../data/derived_data/", 
                            str_replace(params$raw_data, pattern = ".h5ad", replacement = "")))
load(str_c("../../data/derived_data/", 
                            str_replace(params$raw_data, pattern = ".h5ad", replacement = ""),  "/dat.RData"))
```
```{r}
...
saveRDS(fit_result, file = paste0(derived_dat_path, 'fit_result_multi_type_change.rds'))
```
